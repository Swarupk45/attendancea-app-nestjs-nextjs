...
const DropdownUser = () => {
  ...
  console.log("showing id from login to header",userId);
  const router=useRouter();
  const handleId=()=>{
    router.push(`/dashboard/resetpassword/${userId}`)
  }
  ...
  useEffect(() => {
    ...
    const fetchUserProfile = async () => {
      const token = localStorage.getItem('authToken'); 
      console.log("to",token);// Get token from localStorage
      if (!token) {
      ...
      try {
        ...
        console.log("user profile1",token); // Set user data from response
      } catch (err:any) {
    ...
    <ClickOutside onClick={() => setDropdownOpen(false)} className="relative">
      ...
      >
        <span className="hidden text-right lg:block">
          ...
          <span className="block text-xs">{user?.role
